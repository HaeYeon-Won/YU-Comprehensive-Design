...
CONN_DB = pymysql.connect(host='127.0.0.1', user='root', password='passwd', db='pothole', charset='utf8')
...
class ServerSocket: # class for communication with clients
    def __init__(self, ip, port): 
        ...
        self.receiveThread = threading.Thread(target=self.receiveImages) # receive thread start
    ...
    while True:
        for i in SOCKET_DATA: # SOCKET_DATA에 값이 있으면 DB에 저장
            ...
            if jsonObjects.get('msg') == None:
                ...
                LOCK.acquire()
                sql = "INSERT INTO img_path ( img_path, latitude, longitude, addr ) VALUES (%s, %s, %s, %s);" # insert db
...
if __name__ == "__main__":
    save_data_thread = threading.Thread(target=save_data) # save thread start
